...
class ExPingIcmp(threading.Thread):
    def __init__(
            self,
            target,
            interval,
        ...
        super(ExPingIcmp, self).__init__()
...
def log_info(msg):
    collectd.info('ExPingIcmp: ' + msg)

def log_err(msg):
    collectd.error('ExPingIcmp: ' + msg)

def log_warn(msg):
    collectd.warning('ExPingIcmp: ' + msg)
...
def start_monitoring():
    ...
    for target in TARGETS:
        ...
        m = ExPingIcmp(target, INTERVAL, TIMEOUT)
        MONITORS.append(m)
        m.start()
...
def generate_metrics(
    ...
    v = collectd.Values(plugin='exping_icmp', type='exping_icmp',
